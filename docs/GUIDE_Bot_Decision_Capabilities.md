...
> `equity`, `pot_odds.discipline`, and `ranges` (see the matrix above).
> `strong_all_on`'s `ranges: position_aware` now engages because the snapshot
> carries the dealer button; `weak_all_off` stays on `flat`.
...
- **`ranges: position_aware` — done.** `HandState` now carries `button_seat`
  (from `TableStatus.button_seat`) and each seat's `bet` / `is_active`;
  `hand_state_to_snapshot` in `crates/pkdealer_agent_rules/src/main.rs` derives
  `dealer_button` + `logical_seat` from them, so `TableSnapshot::position()`
